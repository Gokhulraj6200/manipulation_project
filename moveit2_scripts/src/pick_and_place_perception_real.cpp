...
private:
  ...
  void result_callback(const GoalHandleFind::WrappedResult &result) {
    ...
    x1_ = (x_ >= 0.343) ? 0.0 : 0.007864;
    y1_ = (y_ >= 0.132) ? 0.0 : 0.012629;
    RCLCPP_INFO(this->get_logger(), "X: %f", x_);
    RCLCPP_INFO(this->get_logger(), "Y: %f", y_);
    RCLCPP_INFO(this->get_logger(), "X1: %f", x1_);
    RCLCPP_INFO(this->get_logger(), "Y1: %f", y1_);
    pick_and_place();
  }

  void setTarget(std::string name, geometry_msgs::msg::Pose &pose,
  ...
  void pick_and_place() {
    ...
    auto move_group_node =
        rclcpp::Node::make_shared("move_group_pick_and_place", node_options);
    // Thread configuration
    rclcpp::executors::SingleThreadedExecutor executor;
    executor.add_node(move_group_node);
    std::thread([&executor]() { executor.spin(); }).detach();
    // Definition of pose targets: position and orientation
    const std::vector<double> PRE_GRASP_POSE{x_ + x1_, y_ + y1_, 0.3, -1.0,
                                             0.00,     0.00,     0.00};
    const std::vector<double> GRASP_POSE{x_ + x1_, y_ + y1_, 0.185, -1.0,
                                         0.00,     0.00,     0.00};
    // Planning interfaces
    ...
    success_gripper = (move_group_gripper.plan(plan_gripper) ==
    ...
    success_arm = (move_group_arm.plan(plan_arm) ==
                   moveit::core::MoveItErrorCode::SUCCESS);
    if (success_arm) {
    ...
    double fraction = move_group_arm.computeCartesianPath(
        grasp_waypoints, eef_step, jump_threshold, trajectory_approach);
    RCLCPP_INFO(this->get_logger(), "Fraction: %.3f.", float(fraction));
    move_group_arm.execute(trajectory_approach);

    // Close gripper
    float gripper_value = 0.630;
    ...
    fraction = move_group_arm.computeCartesianPath(
        reverse_grasp_waypoints, eef_step, jump_threshold, trajectory_approach);
    RCLCPP_INFO(this->get_logger(), "Fraction: %.3f.", float(fraction));
    move_group_arm.execute(trajectory_approach);
    ...
    if (success_arm) {
      RCLCPP_INFO(this->get_logger(),
                  "Plan to rotate first joint 180 degrees: SUCCESS.");
      RCLCPP_INFO(this->get_logger(), "Executing rotation.");
      move_group_arm.execute(plan_arm);
    } else {
      RCLCPP_INFO(this->get_logger(),
                  "Plan to rotate first joint 180 degrees: FAIL.");
